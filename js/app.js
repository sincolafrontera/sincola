// ===================================================
// app.js — Controlador principal SinCola v2.1
// Correcciones: navegación, historial, estado vacío,
// feed, ranking, cookies, formulario contacto
// ===================================================
...
const App = (() => {
  ...
  function startUpdateTimer() {
    ...
    minAgo = 0;
    const el = $('lastUpdate');
    if (el) el.textContent = 'Actualizado ahora mismo';
    updateTimer = setInterval(() => {
      minAgo++;
      const el = $('lastUpdate');
      if (el) el.textContent = `Actualizado hace ${minAgo} min`;
    }, 60000);
  }

  // ── Motor de estado de la frontera ───────────────
  ...
  // ── Historial: mostrar vacío o gráficos ──────────
  function updateHistorico() {
    const tieneReportes = allReportes.length > 0;
    ...
    if (tieneReportes) {
      ...
      // Los charts usan datos estáticos de momento
      // Se poblarán con datos reales cuando haya suficiente historial
    } else {
  ...
  function renderFeed(snapshot) {
    ...
    if (snapshot.empty) {
      ...
      feedEl.appendChild(el('div', { className: 'feed__empty' },
        '🌟 Aún no hay reportes. ¡Sé el primero en reportar!'));
      return;
    }

    // Obtener IDs que ya están renderizados
    ...
    // Añadir los nuevos al principio
    ...
    // Limpiar mensaje de vacío si ya hay items
  ...
  function renderRanking(docs) {
    ...
    if (docs.length === 0) {
      list.appendChild(el('div', { className: 'rank-loading' },
        '¡Aún no hay nadie en el ranking! Reporta para ser el primero.'));
      return;
    }
  ...
  function initRanking() {
    ...
    function loadRanking(type) {
      ...
      semBtn?.setAttribute('aria-selected', type === 'semanal');
      totBtn?.setAttribute('aria-selected', type === 'total');
  ...
  async function loginGoogle() {
    try {
      ...
      // Error específico: contexto no soportado (archivo local)
      if (err.code === 'auth/operation-not-supported-in-this-environment') {
        showToast('Abre la web en internet', 'El login no funciona abriendo el archivo directamente desde el PC', 'error');
      } else {
  ...
  async function sendContact() {
    ...
    // 1. Guardar en Firestore (siempre como backup)
    try {
      await db.collection('contacto').add({
        nombre, email, asunto: asunto || '(sin asunto)', mensaje: msg,
        timestamp: firebase.firestore.FieldValue.serverTimestamp(),
      });
      firestoreOk = true;
    } catch (err) {
    ...
    // 2. Enviar a Formspree para recibir email
    // ⚠️ REEMPLAZA 'YOUR_FORM_ID' con tu ID real de formspree.io
    ...
    if (firestoreOk || formspreeOk) {
      status.className   = 'form-status ok';
      status.textContent = '✅ Mensaje enviado correctamente. Te respondemos lo antes posible.';
      ...
      status.className   = 'form-status err';
      status.textContent = '❌ Error al enviar. Escríbenos directamente a sincola.frontera@gmail.com';
  ...
  const LEGAL_TEXTS = {
    ...
    cookies: `
      ...
      <div style="font-size:.82rem;color:var(--muted);line-height:1.8;margin-top:12px">
        <p style="margin-bottom:12px"><strong style="color:var(--text)">¿Qué son las cookies?</strong><br>Pequeños ficheros que se guardan en tu dispositivo para mejorar tu experiencia y recordar preferencias.</p>
        <p style="margin-bottom:12px"><strong style="color:var(--text)">Cookies que usamos:</strong><br>
          ...
          • <strong style="color:var(--text)">Publicidad:</strong> Google AdSense para mostrar anuncios relevantes que financian el servicio gratuito.
        </p>
        <p style="margin-bottom:16px"><strong style="color:var(--text)">Gestión:</strong><br>Puedes cambiar tus preferencias en cualquier momento desde el botón "Gestionar cookies" del pie de página.</p>
    ...
    terminos: `
      ...
      <div style="font-size:.82rem;color:var(--muted);line-height:1.8;margin-top:12px">
        ...
        <p style="margin-bottom:12px"><strong style="color:var(--text)">Reportes:</strong><br>Deben ser verídicos y basados en observación real. Prohibido: información falsa, spam, bots o sistemas automatizados.</p>
